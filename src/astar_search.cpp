    ...
    void AstarSearch::createStateUpdateTable(int angle_size) {
        ...
        for (int i = 0; i < angle_size; i++) {
            ...
            nu.shift_x = right_circle_center_x +
                         minimum_turning_radius_ * std::cos(M_PI_2 + robot_angle - descretized_angle);
            nu.shift_y = right_circle_center_y +
                         minimum_turning_radius_ * std::sin(M_PI_2 + robot_angle - descretized_angle);
            ...
            nu.shift_x = left_circle_center_x +
                         minimum_turning_radius_ * std::cos(-1.0 * M_PI_2 + robot_angle + descretized_angle);
            nu.shift_y = left_circle_center_y +
                         minimum_turning_radius_ * std::sin(-1.0 * M_PI_2 + robot_angle + descretized_angle);
...
// theta is 0 ~ 2pi
    bool AstarSearch::isGoal(double x, double y, double theta) {
        ...
        if (std::pow(goal_pose_local_.pose.position.x - x, 2) + std::pow(goal_pose_local_.pose.position.y - y, 2) <
            goal_radius) {
    ...
    bool AstarSearch::calcWaveFrontHeuristic(const SimpleNode &sn) {
        ...
        static std::vector<WaveFrontNode> updates = {astar::getWaveFrontNode(0, 1, resolution),
...
//        ROS_INFO("state is obs");
    ...
    bool AstarSearch::findValidClosePose(const grid_map::GridMap &grid_map, const std::string dis_layer_name,
                                         const grid_map::Index &start_index, grid_map::Index &adjusted_index,
        ...
        while (!openlist_.empty()) {
            ...
            for (const auto &state : state_update_table_[sn.index_theta]) {
                ...
                if (isOutOfRange(next.index_x, next.index_y) ||
                    !isSingleStateCollisionFreeImproved(next)/*detectCollision(next)*/)
                    continue;
...
// for debug
    void AstarSearch::publishPoseArray(const ros::Publisher &pub, const std::string &frame) {
        ...
        debug_pose_array_.poses.push_back(start_pose_);
        debug_pose_array_.poses.push_back(goal_pose_);
...
// for demo
    void AstarSearch::broadcastPathTF() {
        ...
        for (unsigned int i = 0; i < path_.poses.size(); i++) {
            ...
            transform.setOrigin(tf::Vector3(path_.poses[i].pose.position.x, path_.poses[i].pose.position.y,
                                            path_.poses[i].pose.position.z));
            transform.setRotation(quat);

            tf_broadcaster_.sendTransform(
                    tf::StampedTransform(transform, ros::Time::now(), path_frame_, "/astar_path"));
